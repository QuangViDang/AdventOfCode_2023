...
func main() {
	...
	strings := strings.Split(strings.TrimSpace(input), "\n")

	total := 0
	for _, str := range strings {
		value := computeChecksum(str)
		total += value
	}

	fmt.Println("Tổng các giá trị hiệu chuẩn là:", total)
}

func computeChecksum(str string) int {
	regex := regexp.MustCompile(`\d|one|two|three|four|five|six|seven|eight|nine`)
	regex2 := regexp.MustCompile(`\d|eno|owt|eerht|ruof|evif|xis|neves|thgie|enin`)
	match1 := regex.FindString(str)
	reversed := reverseString(str)
	match2 := regex2.FindString(reversed)

	if match1 == "" {
		fmt.Println("Không tìm thấy số trong chuỗi.")
		return 0
	}
	result1, err := mapToNumber(match1)
	if err != nil {
		fmt.Printf("Lỗi: %v\n", err)
	} else {
		fmt.Printf("%s: %d\n", match1, result1)
	}
	result2, err2 := mapToNumber(match2)
	if err != nil {
		fmt.Printf("Lỗi: %v\n", err)
	} else {
		fmt.Printf("%s: %d\n", match1, result2)
...
func mapToNumber(value string) (int, error) {
	// Chuyển đổi giá trị thành chữ thường để so sánh không phân biệt chữ hoa chữ thường
	...
	result, found := valueMap[valueLower]
	if !found {
		// Nếu không tìm thấy, trả về lỗi
		return 0, fmt.Errorf("Không tìm thấy giá trị tương ứng cho %s", value)
	}
