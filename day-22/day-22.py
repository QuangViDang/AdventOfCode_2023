...
def parse_input(input_lines) -> list[Bounds]:
    return [Bounds(Vec3(*[int(n) for n in l.split(",")]), Vec3(*[int(n) for n in r.split(",")])) for l, r in (line.split("~") for line in input_lines)]
...
# Solve problems
result_first = solve_first(bricks)
result_second = solve_second(bricks)
