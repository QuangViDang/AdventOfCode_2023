...
def parse_input(input_lines) -> list[tuple[Vec3, Vec3]]:
    return [tuple(Vec3(*[int(n) for n in part.split(",")]) for part in line.split("@")) for line in input_lines]
...
def solve_first(hails) -> object:
    area = (200000000000000, 400000000000000)
    sum = 0
    for h0, h1 in combinations(hails, 2):
        if i := line_intersection(*h0, *h1):
            if area[0] <= i.x <= area[1] and area[0] <= i.y <= area[1]:
                sum += 1
    return sum

def solve_second(hails) -> object:
...
# Parse input
hails = parse_input(input_lines)

# Solve problems
result_first = solve_first(hails)
result_second = solve_second(hails)

# Print results
print("Part 1:", result_first)
print("Part 2:", result_second)
