...
def parse_input(input_lines) -> tuple[list[str], Pos, Pos]:
    map = input_lines
...
# Parse input
map, start, target = parse_input(input_lines)

# Solve problems
result_first = find_longest_path(map, start, target)

# Replace '<' and '>' with '.' in the map for the second problem
map = [re.sub("[<>^v]", ".", row) for row in map]
result_second = find_longest_path(map, start, target)
